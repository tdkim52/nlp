...
def vtag(train, test):
    ...
    for line in train:
        ...
        if tmp[1] not in wtsing_count:
            scount = 0
        else:
            scount = wtsing_count[tmp[1]]
        if line in wordtag_count:
            count = wordtag_count[line] + 1
            if count == 1:
                wtsing_count.update({tmp[1]: (scount + 1)})
            elif count == 2:
                wtsing_count.update({tmp[1]: (scount - 1)})
        ...
        if prevtag not in ttsing_count:
            scount = 0
        else:
            scount = ttsing_count[prevtag]
        if tagtag in tagtag_count:
            count = tagtag_count[tagtag] + 1
            if count == 1:
                ttsing_count.update({prevtag: (scount + 1)})
            elif count == 2:
                ttsing_count.update({prevtag: (scount - 1)})
    ...
    def ttMLE(tag3, tag2):
        ...
        num = num + ((1 + ttsing_count[tag2]) * ttBackoff(tag3))
        denom = tag_count[tag2] + (1 + ttsing_count[tag2])
    ...
    def wtMLE(word, tag):
        ...
        num = num + ((1 + wtsing_count[tag]) * wtBackoff(word))
        denom = tag_count[tag] + (1 + wtsing_count[tag])
        if word == '###' and tag == '###':
            prob = 1.0
        else:
            #prob = float(num + 1) / (denom + vv)
